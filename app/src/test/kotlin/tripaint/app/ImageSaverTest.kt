...
class ImageSaverTest {
    ...
    inner class Save {
        @Test
        fun `returns false if the saver reports failure`() {
        ...
        @Test
        fun `writes image if it does not exist`() {
        ...
        @Test
        fun `overwrites image if it exists and has the same size`() {
            val image = ImageStorage.fill(2, Color.Blue)
            val path = "a.png"
            val location = ImagePool.SaveLocation(File(path))
            val format = SimpleStorageFormat
        ...
        @Test
        fun `overwrites part of image if there already exists a bigger image`() {
            val image = ImageStorage.fill(2, Color.Blue)
            val path = "a.png"
        ...
        @Test
        fun `overwrites part of image if there already exists an image even if it is too small`() {
            val image = ImageStorage.fill(2, Color.Blue)
            val path = "a.png"
            val offset = StorageCoords.from(1, 2)
            val location = ImagePool.SaveLocation(File(path), offset)
