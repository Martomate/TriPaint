...
import com.martomate.tripaint.model.image.{ImagePool, ImageStorage, ImageUtils, RegularImage}
...
class ImageUtilsTest extends FunSuite {
  ...
  test("overwritePartOfImage should write image if it doesn't exist") {
    ...
    val storedImage = ImageUtils.overwritePartOfImage(storage, format, location.offset, None)
  ...
  test("overwritePartOfImage should overwrite part of image if it already exists") {
    ...
    val storedImage =
      ImageUtils.overwritePartOfImage(image, format, offset, Some(existingImage))
