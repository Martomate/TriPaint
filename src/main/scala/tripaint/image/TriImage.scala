...
import tripaint.EditMode
import tripaint.ImagePane
import scalafx.scene.paint.Color
import tripaint.undo.UndoManager
...
class TriImage private(private[image] val storage: ImageStorage, val imagePane: ImagePane) extends Pane {
	...
	def onMouseDragged(e: MouseEvent): Unit = {
  	  ...
  	  val xPos = e.getX()
  		...
  		EditMode.currentMode match {
  		  case EditMode.Orgianize => // TODO: implement scale and rotation if (x, y) is close enough to a corner
  		    if (dragStartedHere) {
    		    panX += xPos - xDrag
    		    panY += yPos - yDrag
        ...
        //		println(s"$steps\t$dist")
        		for (i <- 1 to steps) {
        		  val point = canvas.sceneToLocal(e.getSceneX + xDiff / steps * (i - steps), e.getSceneY + yDiff / steps * (i - steps))
        			mousePressedAt(point.getX, point.getY, e, true)
        		}
  		    }
  		}
  		xDrag = xPos
	...
	def onMousePressed(e: MouseEvent): Unit = {
	  if (!e.isConsumed) {
  		...
  		if (isSelected) mousePressedAt(point.getX, point.getY, e, false)
	  }
	}
	...
	def redraw(doIndexMapping: Boolean): Unit = {
		...
		for (y <- 0 until storage.imageSize) {
			for (x <- 0 until y * 2 + 1) {
				drawTriangle(Coord(x, y)(storage), doIndexMapping, true)
			}
		}
		
		for (y <- 0 until storage.imageSize) {
			for (x <- 0 until y * 2 + 1) {
				drawTriangle(Coord(x, y)(storage), doIndexMapping, false)
			}
		}
	}
	...
	def hasChangedProperty = storage.hasChangedProperty
	
	private val _toolTip = new ReadOnlyObjectWrapper[Tooltip](null, null, new Tooltip("Not saved"))
	def toolTip = _toolTip.readOnlyProperty
