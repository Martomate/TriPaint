...
class MainStage extends PrimaryStage with TriPaintView {
  ...
  override def askForSaveFile(image: TriImage): Option[File] = {
  ...
  override def askSaveBeforeClosing(images: Seq[TriImage]): Option[Boolean] = {
  ...
  private def saveBeforeClosingAlert(images: Seq[TriImage]): Alert = {
